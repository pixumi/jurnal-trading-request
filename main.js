...
function renderTrades() {
    ...
    if (trades.length === 0) {
        // Perbaikan colspan karena tambahan 2 kolom
        historyBody.innerHTML = '<tr><td colspan="11">Belum ada history.</td></tr>';
        calculateAnalytics();
        return;
    }
    ...
    trades.forEach((data, index) => {
      ...
      wlCell.appendChild(wlSpan);
            ...
            // Before Cell (kolom baru)
        const beforeCell = document.createElement('td');
        if (data.before) {
            const beforeLink = document.createElement('a');
            beforeLink.href = data.before;
            beforeLink.target = '_blank';
            beforeLink.textContent = 'View';
            beforeLink.className = 'chart-link';
            beforeCell.appendChild(beforeLink);
        } else {
            const emptySpan = document.createElement('span');
            emptySpan.textContent = '-';
            emptySpan.className = 'empty-chart';
            beforeCell.appendChild(emptySpan);
        }

        // After Cell (kolom baru)
        const afterCell = document.createElement('td');
        if (data.after) {
            const afterLink = document.createElement('a');
            afterLink.href = data.after;
            afterLink.target = '_blank';
            afterLink.textContent = 'View';
            afterLink.className = 'chart-link';
            afterCell.appendChild(afterLink);
        } else {
            const emptySpan = document.createElement('span');
            emptySpan.textContent = '-';
            emptySpan.className = 'empty-chart';
            afterCell.appendChild(emptySpan);
        }
...
form.addEventListener('submit', function(e) {
  ...
  const data = {
    ...
    before: form.before.value.trim(),   // Ambil nilai before
    after: form.after.value.trim()      // Ambil nilai after
  };
...
window.editTrade = function(index) {
  ...
  form.note.value = data.before || '';
  form.note.value = data.after || '';
